...
//    addExclusion("Crafted", Ring);
    ...
    ifstream foodFile("bis/food.txt", ios::in);
    if (!foodFile)
        cout << "Erreur lecture bis/food.txt" << endl;
    while (!foodFile.eof()) {
    ...
    ifstream critMultFile("bis/crit.txt", ios::in);
    if (!critMultFile)
        cout << "Erreur lecture bis/crit.txt" << endl;
