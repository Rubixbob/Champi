...
#pragma region Manual exclusions
...
#pragma endregion
	...
	// Load configuration
	loadConfig("bis/config.txt");

	// Load gear pieces
	loadGearPieces("bis/gear.txt");

	// Initialize full set
	initializeFullSet();

	// Load food data
	loadFoodData("bis/food.txt");

	// Initialize multipliers
	initializeMultipliers("bis/crit.txt");

	cout << "Init done" << endl;
}

void BiSFinder::loadConfig(const string& configFilePath)
{
	ifstream configFile(configFilePath, ios::in);
	if (!configFile)
	{
		cout << "Reading error " << configFilePath << endl;
		return;
	}
...
void BiSFinder::loadGearPieces(const string& gearFilePath)
{
	ifstream file(gearFilePath.c_str(), ios::in);
	if (!file)
	{
		cout << "Reading error " << gearFilePath << endl;
		return;
	}
...
void BiSFinder::initializeFullSet()
{
...
void BiSFinder::loadFoodData(const string& foodFilePath)
{
	ifstream foodFile(foodFilePath, ios::in);
	if (!foodFile) {
		cout << "Reading error " << foodFilePath << endl;
		return;
	}
...
void BiSFinder::initializeMultipliers(const string& critMultFilePath)
{
	ifstream critMultFile(critMultFilePath, ios::in);
	if (!critMultFile) {
		cout << "Reading error " << critMultFilePath << endl;
		return;
	}
...
void BiSFinder::findBiS()
{
	...
	while (pieceCounter[10] < ringPerms || meldFinders.size() > 0) {
		if (active_threads < MAX_THREADS && threadsReady.size() > 0) {
			threadsReady.back()->launch();
			threadsReady.pop_back();
			mutex_active_threads.lock();
			active_threads++;
			mutex_active_threads.unlock();
		}
		else if (threadsReady.size() < MAX_THREADS && pieceCounter[10] < ringPerms) {
			...
			if (!m_full_set.gear_set[10]->name.compare("Crafted") || m_full_set.gear_set[10]->name.compare(m_full_set.gear_set[11]->name)) {
				if (m_full_set.gearStats.subs[3] <= MAX_SKS && m_full_set.gearStats.subs[3] + sksSlotsAvailable * MAT + foodMaxSks >= MIN_SKS) {
					...
					threadsReady.push_back(new sf::Thread(&MeldFinder::findBestMelds, meldFinder));
	...
	cout << "Solving done in: " << (int)(elapsed.asSeconds() / 60) << "m" << setw(2) << (int)elapsed.asSeconds() % 60 << "s" << setw(3) << elapsed.asMilliseconds() % 1000 << endl
